...
my $p1 = { fsprefix => 'pmcl01' };
my $p2 = { fsprefix => 'pmcl02' };
my $no = {};

my @t = (
  # to-array
  ['to  pmcl01 disk',  $A->($p1,'vm-124-disk-0'),        'pmcl01-vm-124-disk-0'],
  ['to  pmcl01 state', $A->($p1,'vm-124-state-snap1'),   'pmcl01-vm-124-state-snap1'],
  ['to  no-prefix',    $A->($no,'vm-124-disk-0'),        'vm-124-disk-0'],
  ['to  empty prefix', $A->({fsprefix=>''},'vm-1-disk-0'), 'vm-1-disk-0'],
  # from-array, own
  ['fr  own',          $V->($p1,'pmcl01-vm-124-disk-0'), 'vm-124-disk-0'],
  ...
  ['obs disk',         $A->($p1,'vm-102-disk-0'),       'pmcl01-vm-102-disk-0'],
  ['obs state',        $A->($p1,'vm-102-state-test'),   'pmcl01-vm-102-state-test'],
  ['obs snap',         SNAPNAME($A->($p1,'vm-102-disk-0'),'test'), 'pmcl01-vm-102-disk-0.test'],
  ['obs disk  back',   $V->($p1,'pmcl01-vm-102-disk-0'),     'vm-102-disk-0'],
  ['obs state back',   $V->($p1,'pmcl01-vm-102-state-test'), 'vm-102-state-test'],
);
my $fail = 0;
for my $c (@t) {
  my ($name,$got,$want) = @$c;
